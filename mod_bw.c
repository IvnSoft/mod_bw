...
static int handle_bw(request_rec *r)
{
    ...
    /* Only work on main request/no subrequests and no redirets */
    if (r->main || r->prev)
        return DECLINED;
...
static int bw_filter(ap_filter_t *f, apr_bucket_brigade *bb)
{
    ...
    char sizebuf[5][5];                     /* buffers for human-readable size/rate conversion */
    ...
    /*
       ...
       - Read the content of the bucket, and send it to the next filter, piece by piece
     */
    while (b != APR_BRIGADE_SENTINEL(bb)) {
        ...
        if (apr_bucket_read(b, &buf, &bytes, APR_NONBLOCK_READ) ==
            ...
            while (bytes > 0) {
                ...
                if (sleep < 10000 && (ctx->client_bw > cur_rate || ctx->client_bw == 0))
                {
                    ...
                    if (ctx->sleep_bypasses_left == 0) {                      /* If not currently counting down packets... */
                        ...
                        if (ctx->sleep_bypasses_total != current_sleep_bypasses) {
                            /* new packet counter = current_sleep_bypasses minus packets already sent */
                ...
                if (ctx->sleep_bypasses_left > 0) {
                    ...
                    /* Only sleep if the client's calculated bw is greater than available or client bw not yet known */
                    if (ctx->client_bw > cur_rate || ctx->client_bw == 0) {
                        apr_sleep(sleep);
